...
const CATALOG = {
  ...
  employee_missing_location_policy:{category:'Location',title:'Employee missing location policy',severity:'warning',why:'An active salesman has no employee-specific location policy row.'},
  tracking_enabled_gps_disabled:{category:'Location',title:'Tracking enabled while GPS is off',severity:'warning',why:'Continuous GPS tracking is enabled for an employee whose GPS Location setting is off.'},
  mandatory_lead_location_gps_disabled:{category:'Location',title:'Mandatory lead location while GPS is off',severity:'warning',why:'Location is marked mandatory for new leads even though GPS Location is disabled for that employee.'},
  active_tracking_no_pings:{category:'Location',title:'Active employee has no GPS pings',severity:'warning',why:'The employee has an active Start Day and continuous tracking enabled, but no location ping has been recorded for this active session.'},
  active_tracking_stale:{category:'Location',title:'Active employee location is stale',severity:'warning',why:'The employee has an active Start Day and continuous tracking enabled, but the latest GPS ping is more than 15 minutes old.'},
...
const CHECKS = {
  ...
  employee_missing_location_policy:`SELECT count(*) FROM users u LEFT JOIN employee_day_closing_permissions p ON p.user_id=u.id WHERE u.role='salesman' AND u.is_active=true AND p.user_id IS NULL`,
  tracking_enabled_gps_disabled:`SELECT count(*) FROM employee_day_closing_permissions p JOIN users u ON u.id=p.user_id WHERE u.role='salesman' AND u.is_active=true AND p.continuous_gps_tracking=true AND p.gps_location=false`,
  mandatory_lead_location_gps_disabled:`SELECT count(*) FROM employee_day_closing_permissions p JOIN users u ON u.id=p.user_id WHERE u.role='salesman' AND u.is_active=true AND p.location_mandatory_for_new_lead=true AND p.gps_location=false`,
  active_tracking_no_pings:`SELECT count(*) FROM attendance a JOIN users u ON u.id=a.salesman_id JOIN employee_day_closing_permissions p ON p.user_id=a.salesman_id WHERE a.start_day_at IS NOT NULL AND a.end_day_at IS NULL AND u.is_active=true AND p.gps_location=true AND p.continuous_gps_tracking=true AND NOT EXISTS (SELECT 1 FROM location_pings lp WHERE lp.salesman_id=a.salesman_id AND lp.captured_at>=a.start_day_at)`,
  active_tracking_stale:`SELECT count(*) FROM attendance a JOIN users u ON u.id=a.salesman_id JOIN employee_day_closing_permissions p ON p.user_id=a.salesman_id WHERE a.start_day_at IS NOT NULL AND a.end_day_at IS NULL AND u.is_active=true AND p.gps_location=true AND p.continuous_gps_tracking=true AND EXISTS (SELECT 1 FROM location_pings lp WHERE lp.salesman_id=a.salesman_id AND lp.captured_at>=a.start_day_at) AND (SELECT max(lp.captured_at) FROM location_pings lp WHERE lp.salesman_id=a.salesman_id AND lp.captured_at>=a.start_day_at) < now()-interval '15 minutes'`,
...
const DETAIL = {
  ...
  employee_missing_location_policy:`SELECT u.id,u.full_name AS employee,u.is_active FROM users u LEFT JOIN employee_day_closing_permissions p ON p.user_id=u.id WHERE u.role='salesman' AND u.is_active=true AND p.user_id IS NULL ORDER BY u.full_name LIMIT 50`,
  tracking_enabled_gps_disabled:`SELECT u.id,u.full_name AS employee,p.gps_location,p.continuous_gps_tracking FROM employee_day_closing_permissions p JOIN users u ON u.id=p.user_id WHERE u.role='salesman' AND u.is_active=true AND p.continuous_gps_tracking=true AND p.gps_location=false ORDER BY u.full_name LIMIT 50`,
  mandatory_lead_location_gps_disabled:`SELECT u.id,u.full_name AS employee,p.gps_location,p.location_mandatory_for_new_lead FROM employee_day_closing_permissions p JOIN users u ON u.id=p.user_id WHERE u.role='salesman' AND u.is_active=true AND p.location_mandatory_for_new_lead=true AND p.gps_location=false ORDER BY u.full_name LIMIT 50`,
  active_tracking_no_pings:`SELECT a.id AS attendance_id,a.day,a.start_day_at,u.full_name AS employee,p.gps_location,p.continuous_gps_tracking FROM attendance a JOIN users u ON u.id=a.salesman_id JOIN employee_day_closing_permissions p ON p.user_id=a.salesman_id WHERE a.start_day_at IS NOT NULL AND a.end_day_at IS NULL AND u.is_active=true AND p.gps_location=true AND p.continuous_gps_tracking=true AND NOT EXISTS (SELECT 1 FROM location_pings lp WHERE lp.salesman_id=a.salesman_id AND lp.captured_at>=a.start_day_at) ORDER BY a.start_day_at LIMIT 50`,
  active_tracking_stale:`SELECT a.id AS attendance_id,a.day,a.start_day_at,u.full_name AS employee,(SELECT max(lp.captured_at) FROM location_pings lp WHERE lp.salesman_id=a.salesman_id AND lp.captured_at>=a.start_day_at) AS last_ping FROM attendance a JOIN users u ON u.id=a.salesman_id JOIN employee_day_closing_permissions p ON p.user_id=a.salesman_id WHERE a.start_day_at IS NOT NULL AND a.end_day_at IS NULL AND u.is_active=true AND p.gps_location=true AND p.continuous_gps_tracking=true AND EXISTS (SELECT 1 FROM location_pings lp WHERE lp.salesman_id=a.salesman_id AND lp.captured_at>=a.start_day_at) AND (SELECT max(lp.captured_at) FROM location_pings lp WHERE lp.salesman_id=a.salesman_id AND lp.captured_at>=a.start_day_at) < now()-interval '15 minutes' ORDER BY last_ping LIMIT 50`,
