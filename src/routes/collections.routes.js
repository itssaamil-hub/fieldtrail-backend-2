...
router.put('/:key/due-date',async(req,res)=>res.json(await C.transaction(async query=>{let a=await C.getAccount(query,req.user,req.params.key,true);if(req.body.version!==a.version)throw bad('Account changed. Refresh before saving.',409);a=await C.materialize(query,a);await query('UPDATE collection_accounts SET due_date=$2,version=version+1 WHERE id=$1',[a.id,date(req.body.dueDate,true)]);return {key:a.id};})));
router.post('/:key/payments',async(req,res)=>{if(!UUID.test(req.body.requestId))throw bad('Payment request identifier required');res.status(201).json({payment:await C.record(req.user,req.params.key,req.body)});});
