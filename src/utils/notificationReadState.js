...
async function getUnread(user, query = require('../db').query) {
  ...
  const { rows: taskCounts } = await query(`SELECT count(*)::integer AS count FROM task_notifications n JOIN crm_tasks t ON t.id=n.task_id WHERE n.user_id=$1 AND n.read_at IS NULL AND t.status IN ('pending','in_progress')`, [user.id]);
