...
test('employee due list is filtered before pagination; admin status is validated',async()=>{
 ...
 db.query=async(sql,p)=>{if(sql.includes('FROM users WHERE id=$1'))return{rows:[{id:p[0],role:p[0]==='22222222-2222-4222-8222-222222222222'?'admin':'salesman',full_name:'Test User',is_active:true,auth_version:0}]};sqlSeen=sql;args=p;return{rows:[{accounts:[],summary:{collected:0,pending:0,overdue:0},employees:[]}]}};
 ...
 const id='11111111-1111-4111-8111-111111111111',admin='22222222-2222-4222-8222-222222222222';const call=(role,status)=>fetch(`http://127.0.0.1:${server.address().port}/collections?status=${status}&offset=50`,{headers:{Authorization:'Bearer '+signToken({id:role==='admin'?admin:id,role})}});
