...
const {validateTemplate,buildSummary,snapshotSteps}=require('../src/utils/onboarding');
...
test('onboarding HTTP authorization, snapshots, revisions, progress, summary and PDF',async()=>{
 let template={version:1,steps:[{id:'setup',title:'Account setup'},{id:'training',title:'Staff training'}]},saved=null,won=true;const calls=[];
 ...
 if(sql.startsWith('SELECT version,steps FROM onboarding_template'))return{rows:[structuredClone(template)]};
 if(sql.startsWith('UPDATE onboarding_template')){if(p[1]!==template.version)return{rows:[]};template={version:template.version+1,steps:JSON.parse(p[0])};return{rows:[structuredClone(template)]}}
 ...
 if(sql.startsWith('SELECT c.*,l.id'))return{rows:!saved||p[1]&&p[1]!==ids.sam?[]:[{...structuredClone(saved),...lead}]};
 ...
 assert.equal((await req('admin','/template','PUT',{version:1,steps:[{id:'new',title:'New template step'}]})).status,200);
 ...
 const summary=await(await req('admin','/'+ids.lead+'/summary')).json();assert.ok(summary.summary.text.includes('Dubai Darbar'));assert.ok(!JSON.stringify(summary).includes('Private setup note'));
